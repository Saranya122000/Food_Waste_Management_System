...
{	
    ...
    // console.log(JSON.stringify(data));
    	if (data.results.length > 0) {
      	...
      	lat=place.geometry.lat;
      	lng=place.geometry.lng;
    	var sql="SELECT d_phone,d_email,ver_status FROM donor WHERE d_username='" + un + "'";
  			con.query(sql, function (err, result) 
  			{
    			if (err) throw err;
...
app.post("/register", (req, res) => 
{
	if(req.body.submit)
	{
	   ...
	   if(user=="donor")
  		{
  			var sql = "INSERT INTO donor(d_username,d_name,d_phone,d_addr,d_email,d_password) VALUES('" + un + "','" + name + "','" + phone + "','" + addr + "','" + email + "','" + pwd +"')";
  			con.query(sql, function (err, result) 
  			{
    			if (err) 
    			{	
